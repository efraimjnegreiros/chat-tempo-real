...
export default function Home({ currentUser, logout }) {
  ...
  useEffect(() => {
    ...
    api.patch(`/users/${currentUser.id}`, { online: true, lastSeen: null });
    ...
    // Marca como offline ao fechar/sair da aba
    const handleOffline = () => {
      api.patch(`/users/${currentUser.id}`, {
        online: false,
    ...
    document.addEventListener("visibilitychange", () => {
      if (document.hidden) {
        api.patch(`/users/${currentUser.id}`, { online: false, lastSeen: new Date().toISOString() });
      } else {
        api.patch(`/users/${currentUser.id}`, { online: true, lastSeen: null });
        document.title = "Chat 💬";
      }
